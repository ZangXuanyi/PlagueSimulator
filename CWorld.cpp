...
namespace Ethene
{
	...
	{
        std::string path = "..\\..\\source\\countryInfo.csv";
        ...
        for (std::string& CountryInfo : CountryInfoAll)
		{
            ...
            c.researchInvestmentTotal = std::stod(ci[2])*1000;
			...
			c.deadPopulation = 0;
			c.areaOrder = 1;
            c.changeToLocalOrder = 0;
            c.changeToResearchInvestment=0;
            c.changeToLocalCorpseTransmission=0;
            c.changeToLocalInfectivity=0;
            c.changeToLocalLethality=0;
            c.changeToLocalSeverity=0;
	...
	{
        std::string path = "..\\..\\source\\policyInfo.csv";
        ...
        for (std::string& PolicyInfo : PolicyInfoAll)
		{
            ...
            p.changeToBorder = std::stoi(pi[13]);
	...
	void CWorld::Update(const CDisease &disease)
	{
		...
		{
            if(country.healthyPopulation+country.infectedPopulation==0) continue;
			country.Update(world, disease);
            ar += country.GetCurrentRealMedicalInput(disease);
		}
        world.worldTotalResearch += ar;
        ...
        researchHistory.push_back(*new std::tuple<long,long,long>(disease.cureRequirement, world.worldTotalResearch,ar));
	}
}
