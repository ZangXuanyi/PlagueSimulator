...
namespace Ethene
{
	...
	double CCountry::GetCurrentAreaAttention(CWorld& world, const CDisease& disease)
	{
		...
		double infectedSense =
            1.5 * infectedRatio * infectedRatio * areaSeverityRatio * areaSeverityRatio + areaSeverityPower;
		double deathSense =
	...
	{
        if (healthyPopulation<=0||infectedPopulation==0)
		{
			return 0;
		}
	...
	long CCountry::AddDeath(const CDisease& disease)
	{
		...
		{
            long delta = (disease.lethality + changeToLocalLethality) * GetRandomNumber(5, 10) * infectedPopulation / 6000;
	...
	void CCountry::CrossBorder(CWorld& world, const CDisease& disease) const
	{
		...
		for (CCountry& country : world.countries)
		{
			if (country.name == name)
                continue;
            else if (country.infectedPopulation > 0||country.healthyPopulation<=0)
                continue;
            else
            {
